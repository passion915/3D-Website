...
var openMediaResObj;
var mapModalEl;
var hotSpotModalTitle = ["添加全景切换热点","添加超链接热点","添加图册","添加文本热点","添加音乐","添加图文热点","添加分组","添加环物热点","添加视频热点"];
...
$(function () {
    ...
    $(document).on('click','.edit-item .caption > span',function(e){
        ...
        if($(this).hasClass('group-location-icon')){//定位
            ...
            resetHotspotSum();
        }else if($(this).hasClass('group-view')){
            scene_old = krpano.get('xml.scene')
            hotspotname_old = hotspotName;
            var linkscene = $(this).data('linkscene');
            var data = $('#panoSettingModal .hot').data(scene_old);
            var hotspotArr = data[hotSpotDataKey[0]];
            var currentHotspot ;
            $(hotspotArr).each(function(){
                if(this.name == hotspotName){
                    currentHotspot = this;
                }
            })
            if(currentHotspot.hlookat!==undefined){
                krpano.call("gotoscene('"+linkscene+"', "+currentHotspot.hlookat+","+currentHotspot.vlookat+");");
            }else
                krpano.call("gotoscene('"+linkscene+"', 0,0);");
...
function addImageMediaRes(data,isPreAppend) {
    $(data).each(function (idx) {
       var thumbnail = global_storage=='qiniu'?"?imageView2/2/w/100":"?x-oss-process=image/resize,m_lfit,h_100,w_100";
...
$(document).ready(function () {
    ...
    $(".pano-setting-container button").click(function () {
        ...
        if(n == 5) {
            ...
            $('.mtime-square_eye').hide();
            $('.img-active').removeClass('img-active');
            // 重新载入父场景
            var parent_scene = $('.'+krpano.get('xml.scene')).parents('.parent-scene').attr('p-scene');
            var krpano = document.getElementById("panoSettingObject");
            krpano.call('loadscene('+parent_scene+')');
        }
    });
     ...
     window.onload = function(){
      ...
      // var krpano = ;
      $(".mtime").on("click",".thumb",function(){
          var scene_name = $(this).attr('name');
          console.log('当前：'+scene_name);
          $('.mtime-square_eye').show();
          $('.img-active').removeClass('img-active');
          $(this).find('img').addClass('img-active');
          document.getElementById('panoSettingObject').call('loadscene('+scene_name+')'); 
      });
    }
...
function initPage(data) {
    ...
    //初始化标签框
    init_tag_chose("pid_edit_tag_chosen",1);
    var tag_list = data.tag_list;
    var tags = data.tags;
    var custom_tag_list = data.custom_tag_list;
    var custom_tags = data.custom_tags;
    ...
    $("#pid_edit_tag_chosen .tag_list").html(chose_list_html);
    $("#pid_edit_tag_chosen .chosen-choices .search-field").before(chose_html);
...
function updateWorks() {
    ...
    var tags = get_chose_tags("pid_edit_tag_chosen");
    worksMaindata.tags = tags.default;
    worksMaindata.custome_tags = tags.custom;
    ...
    if($("#region_wrap").get(0)){
        sb.pushData("regions",getRegions());
    }
    if($("#work_property_wrap").get(0)){
        sb.pushData("properties",getProperties());
    }
...
function dataEach(data){
    ...
    $.each(data,function (idx) {
        ...
        if (checked) {
         ...
         var thumbnail = global_storage=='qiniu'?"?imageView2/2/w/36":"?x-oss-process=image/resize,m_lfit,h_36,w_36";
    ...
    if (mediaType == '1') {
        ...
        if(multiselect){
            ...
            $("#pic img.img-selected").each(function(idx){
                ...
                mediaObj.src = interceptImg($(this).attr("src"));
            ...
            mediaData.src = interceptImg($("#pic img.img-selected").attr("src"));
        }
    }
...
function setInitView2(){
    ...
    var hotspotArr = data[hotSpotDataKey[0]];
    $(hotspotArr).each(function(){
        if(this.name == hotspotname_old){
            this.hlookat = hlookat;
            this.vlookat = vlookat;
        }
    })
    $('#panoSettingModal .hot').data(scene_old,data);
...
function backToScene(){
     var krpano = document.getElementById('panoSettingObject');
      krpano.call('loadscene('+scene_old+', null, MERGE);');
      $(".square_eye2").hide();
}
...
function setMtimeInitView() {
    ...
    putMtimeSetting(sceneName,hlookat,vlookat);
    
}

function putMtimeSetting(sceneName,hlookat,vlookat){
    // 获取父场景视角数据
    var krpano = document.getElementById('panoSettingObject');
    console.log('child: '+krpano.get('xml.scene'));
    var parent_scene = $('.'+krpano.get('xml.scene')).parents('.parent-scene').attr('p-scene');
    console.log('parent : '+parent_scene);
    // var parent_scene = 'scene_5c8fb4545aaa2119';

    var lookatObj = $("#panoSettingModal .eyes").data(parent_scene);
    if(lookatObj == undefined){
        $.zui.messager.show("未获取到父场景的角度信息", {placement: 'top', type: 'error', time: '3000', icon: 'check'});
        return false;
    }
    var children_view = {};
    children_view.ath_offset = hlookat - lookatObj.hlookat;
    children_view.atv_offset = vlookat - lookatObj.vlookat;
    children_view.ath_parent = lookatObj.hlookat;
    children_view.atv_parent = lookatObj.vlookat;
    children_view.scene_name = sceneName;

    console.log(children_view);
    $.get('/edit/pic', {children_view, 'act':'save_children_view', 'pk_works_main': pk_works_main}, function(e){
        e = JSON.parse(e);
        if(e.flag == 1){
            $.zui.messager.show("初始视角已设置", {placement: 'top', type: 'success', time: '3000', icon: 'check'});
        }else{
            $.zui.messager.show("设置失败", {placement: 'top', type: 'error', time: '3000', icon: 'check'});
        }
    })
}
        ...
        switch(idx){
            ...
            //console.log('addSceneChangeHotSpot("'+param.imgPath+'","'+ (param.name) +'",'+param.linkedscene+','+(param.ath)+','+(param.atv)+','+param.isDynamic+','+settingFlag+','+isBtnClicked(1)+')');
                ...
                mthml = '<div class="edit-item" data-respk="'+param.name+'">' +
                    '<a class="card">' +
                    '<span class="caption">' +
                    '<span class="group-location-icon">定位</span><span class="group-change-icon" data-liindex="0" data-iconType="'+param.iconType+'" data-hotspotidx="0">更换图标</span>&nbsp;&nbsp;<span class="group-rename"  data-iconType="'+param.iconType+'" data-liindex="1" data-hotspotidx="0">场景&nbsp;&nbsp;</span><span class="group-delte">删除</span><span class="group-view" data-linkscene="'+param.linkedscene+'">视角</span>' +
                    '</span> </a> ' +
                    '<div class="group-item-title"> ' +
...
function initViewSetting(sceneName){
    ...
    if(lookatObj){
        if(settingFlag){
            ...
            ex1Slider.slider("setValue",1*lookatObj.fov,true);
            ex2Slider.slider("setValue",[1*lookatObj.fovmin,1*lookatObj.fovmax],true);
            ex3Slider.slider("setValue",-1*lookatObj.vlookat,true);
            ex4Slider.slider("setValue",[1*lookatObj.vlookatmin,1*lookatObj.vlookatmax],true);
...
function choose_imgtext_ok(mediaData){
    imgtext_editor.appendHtml("<img src='"+mediaData.src+"' />");
