...
$(function(){
    plugins_init_function.push(cus_map_annotation_init);
})
function cus_map_annotation_init(){
    $.post('/plugin/map_annotation/map_annotation.php',
            {
                'act' : 'get',
                'wid' : pk_works_main
            },function(data){
                if (data.id)
                    map_annotation_param = data;
            },'json');
}
...
function map_annotation_add(){
    ...
    map_annotation_param.pic_tags = tags.default;
	var krpano = document.getElementById('map_annotation_object');
	map_annotation_param.cus_hotspot_title=$.trim($("#worksname").val());
...
function map_annotation_cus_ok(){
    if (!map_annotation_param.cus_wid) {
        alert_notice('请先添加标注');
        return false;
    }
	var sb = _U.getSubmit("/plugin/map_annotation/map_annotation.php", null, "ajax", false);
	sb.pushData("act","doAdd");
	sb.pushData("param",map_annotation_param);
	sb.submit(function () {

    }, function (data) {
        if (data.status==1) {
            alert_notice('标注成功','success');
        }else{
...
function map_a_remove(){
    ...
    sb.pushData("cus_wid",map_annotation_param.cus_wid);
    sb.submit(function () {
