<?php 
	...
	*/
	...
	if($act == 'get_pano_config'){
		...
		exit;
	}else if($act == 'get_map_by_tag'){
		$tag = intval($_REQUEST['tag']);
		// $config = $Db->query('SELECT p.hotspot,w.pk_works_main,w.view_uuid FROM '.$Base->table('pano_config').' p LEFT JOIN '.$Base->table('worksmain').' w ON p.pk_works_main =  w.pk_works_main LEFT JOIN '.$Base->table('tag_works').' t ON t.works_id = w.pk_works_main WHERE w.pk_user_main = '.$_lang['map_uid'].' AND t.tag_id='.$tag,'Row');
		$config = $Db->query('SELECT p.hotspot,w.pk_works_main,w.view_uuid FROM '.$Base->table('pano_config').' p LEFT JOIN '.$Base->table('worksmain').' w ON p.pk_works_main =  w.pk_works_main  WHERE w.pk_user_main = '.$_lang['map_uid'],'Row');
		echo $Json->encode($config);
		exit;
	}else if($act == 'doAdd'){
		$re['status'] = 0 ;
		...
		$list = $Db->query('SELECT * FROM '.$Base->table('map_annotation').' WHERE  sys_scene = "'.$scene.'"');
		if (empty($list)) {
			$list = array();
		}
