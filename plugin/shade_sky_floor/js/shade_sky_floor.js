...
function bulid_shade_sky_floor(panoConfig){
	var shadeObj = {};
	var isWhole = $("input[name=radioOptionsExample]:checked").val() == '0'?1:0 ;
	shadeObj.isWhole = isWhole;
	if (isWhole) {
	    var useShade = $("input[name=radio1]:checked").val() == '0'?0:1;
	    shadeObj.useShade = useShade;
	    shadeObj.shadeSetting = {};
	    if (useShade) {
	        var isDefault = $("input[name=radio1]:checked").val() == '1'?1:0;
	    ...
	    $("#oneDiv > div").each(function (idx) {
	        var setting = {};
	        setting.useShade = $(this).find(".radio input[name=radioUse" + idx + "]:checked").val() == '0'?0:1;
	        if (setting.useShade) {
	            setting.isDefault = $(this).find(".radio input[name=radioUse" + idx + "]:checked").val() == '1'?1:0;
