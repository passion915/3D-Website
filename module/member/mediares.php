...
//编辑项目时，请求图标素材
	...
	if (!empty($input)) {
		$type = Common::sfilter($input['type']);
		$media_type=Common::sfilter($input['media_type']);
	}else{
		$type = Common::sfilter($_REQUEST['type']);
		$media_type=Common::sfilter($_REQUEST['media_type']);
	}
...
//素材管理页面，按类别获取素材
	...
	switch ($type) {
		case 1:
			...
			$sql = ' FROM '.$Base->table("imgsmain").' i WHERE 1=1 ';
			$extParams['name'] = Common::sfilter($extParams['name']);

			if(isset($extParams['atlasId']))
				$sql.=' AND i.pk_atlas_main = '.intval($extParams['atlasId']);
			...
			// 	$sql.=' WHERE i.pk_atlas_main = '.$extParams['atlasId'];
			// }else{
...
// 获取未分配全景图片
else if($act == 'list_imgs_new') {
	$p_id = intval($_REQUEST['p_id']);
	$sql = 'SELECT pk_img_main as value, filename as text, parent_img FROM u_imgsmain WHERE type != 1 AND pk_img_main != '.$p_id .' AND pk_user_main ='. $user['pk_user_main'] .' ORDER BY pk_img_main desc';
	$res = $Db->query($sql);
	$re;
	foreach ($res as $key => $value) {
		if($value['parent_img'] == $p_id){
			$value['status'] = true;
		}else{
			$value['status'] = false;
		}
		if(!empty($value['parent_img']) && $value['parent_img'] != $p_id){
		}else{
			$re[] = $value;
		}
	}
	...
	if(empty($parent_id) ){
		...
		$Db->update('u_imgsmain', ['parent_img'=>0, 'type'=> 0], array('parent_img'=>$parent_id));

		if(!empty($child_id)){
			$Db->update('u_imgsmain', ['type'=>'1'], ['pk_img_main'=>$parent_id]);
			foreach ($child_id as  $value) {
				$Db->update('u_imgsmain', ['type'=>'2', 'parent_img'=>$parent_id], ['pk_img_main'=>$value]);
			}
		}else{
			$Db->update('u_imgsmain', ['type'=>'0'], ['pk_img_main'=>$parent_id]);
		}
		$res['status'] = 1;
	}
...
//资源删除
	...
	switch ($type) {
		case 1:
			$info = $Db->query('SELECT type FROM u_imgsmain WHERE pk_user_main = '. $user['pk_user_main'] .' AND pk_img_main = '. $media_id, 'One');
			if($info != 0){
				$re['msg'] = "资源被关联，不能删除";
			}
			elseif($Db->getcount($Base->table("imgs_works"),"id",array("pk_img_main"=>$media_id))>0){
				$re['msg'] = "资源被引用，不能被删除";
			}else{
...
//替换全景图片封面
	...
	$cover = Common::sfilter($_REQUEST['thumb_path']);
	$mid = intval($_REQUEST['mid']);
	$img = $Db->query('SELECT view_uuid FROM '.$Base->table('imgsmain').' WHERE pk_img_main='.$mid,'Row');
	$re['status'] = 0;
	if (empty($img)) {
		$re['msg'] = '未查询到相关的全景素材';
	}else{
		$Db->update($Base->table('imgsmain'),array('thumb_path'=>$cover),array('pk_img_main'=>$mid,'pk_user_main'=>$user['pk_user_main']));
		$panos = $Db->query('SELECT pk_pano_config,scene_group FROM '.$Base->table('imgs_works').' AS i LEFT JOIN '.$Base->table('pano_config').' AS p ON p.pk_works_main = i.pk_works_main WHERE i.pk_img_main = '.$mid);
		foreach ($panos as $p) {
			$flag = false;
			if (!empty($p['scene_group'])) {
				$scene_group = $Json->decode($p['scene_group']);
				if (!empty($scene_group['sceneGroups'])) {
					foreach ($scene_group['sceneGroups'] as &$sceneGroups) {
						if ($sceneGroups['scenes']) {
							foreach ($sceneGroups['scenes'] as &$s) {
								if ($s['viewuuid']==$img['view_uuid']) {
									$s['imgPath'] = $cover;
									$flag = true;
									break;
								}
					...
					$Db->update($Base->table('pano_config'),array('scene_group'=>$Json->encode($scene_group)),array('pk_pano_config'=>$p['pk_pano_config']));
				}
			}
		}
		$re['status'] = 1;
