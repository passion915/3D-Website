...
if($act=="list_scenes"){
	...
	$res = list_scenes( $page,$pageSize,$user['pk_user_main']==$_lang['map_uid']?0:$user['pk_user_main']);
...
//删除项目
	...
	if($limit_num = $Db->query("select limit_num from ".$Base->table('user')." where pk_user_main=".$user['pk_user_main']."","One")){
		...
		try{
			...
			//删除项目关联的地图标注
			$Db->delete($Base->table('map_annotation'),array('cus_wid'=>$pid));
			$Db->commit();
			$re['flag'] = 1;
		}catch(Exception $e){
			$Db->rollback();
...
//删除一个视频项目
	...
	$re['browser'] = $Db->query('SELECT * FROM u_browser WHERE pk_works_main = '.$pk_works_main, 'Row');
	$re['browser'] = $re['browser'] == false ? [] : $re['browser'];

	$re['province'] = $Db->query('SELECT region_name as name, region_count as value FROM u_region_sts WHERE type = 1 AND pk_works_main = '.$pk_works_main . ' LIMIT 0, 5');
	foreach ($re['province'] as  $value) {
		$re['pro_list'][] = $value['name'];
	}

	$re['city'] = $Db->query('SELECT region_name as name, region_count as value FROM u_region_sts WHERE type = 2 AND pk_works_main = '.$pk_works_main . ' LIMIT 0, 5');
	foreach ($re['city'] as  $value) {
		$re['city_list'][] = $value['name'];
	}

	$temp = $Db->query('SELECT u.date as date, COUNT(*) as value  FROM ( SELECT DATE_FORMAT(vistortime, "%Y-%m-%e") as date FROM u_vistor_works WHERE pk_works_main = '.$pk_works_main.' ) u  GROUP BY u.date ORDER BY date DESC LIMIT 7');

	foreach ($temp as $key => $value) {
		$re['uv']['key'][] = $value['date'];
		$re['uv']['value'][] = $value['value'];
	}

	$re['total'] = $Db->query('SELECT browsing_num FROM u_worksmain WHERE pk_works_main = '.$pk_works_main, 'One');
